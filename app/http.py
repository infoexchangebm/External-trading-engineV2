...
async def post_json(
    ...
    *,
    ...
    """POST ``json`` to ``url`` and return decoded JSON, retrying transient failures.

    Mirrors :func:`get_json`'s retry/backoff behaviour for outbound writes.
    ...
    try:
        for attempt in range(1, attempts + 1):
            try:
                response = await http.post(url, json=json, headers=headers, timeout=request_timeout)
                if response.status_code in RETRYABLE_STATUS:
                    raise UpstreamError(f"{url} returned retryable status {response.status_code}")
                response.raise_for_status()
                return response.json()
                ...
                backoff = min(2 ** (attempt - 1) * 0.5, 8.0)
                logger.warning(
                    "POST %s failed (attempt %s/%s): %s - retrying in %.1fs",
                    url,
                    attempt,
                    attempts,
                    exc,
    ...
    raise UpstreamError(f"POST {url} failed after {attempts} attempt(s): {last_error}")
